...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        for video, index_range in self.video_dir_dict.items():
            if idx >= index_range[0] and idx < index_range[1]:
                ...
                img_path = os.path.join(
                    self.video_dir,
                    video,
                    os.listdir(os.path.join(self.video_dir, video))[img_index],
                )
                ...
                ref_path = os.path.join(
                    self.video_dir,
                    video,
                    os.listdir(os.path.join(self.video_dir, video))[ref_index],
                )
        ...
        if self.transform:
            ...
            img_spatial_aug = (np.array(img_spatial_aug) / 255.0).astype(np.float32)
            img_pixel_aug = (np.array(img_pixel_aug) / 255.0).astype(np.float32)
            ref_rgb = (np.array(ref_rgb_aug) / 255.0).astype(np.float32)

            img_l = cv2.cvtColor(img_pixel_aug, cv2.COLOR_RGB2LAB)[:, :, 0] / 100.0
            img_ab = cv2.cvtColor(img_spatial_aug, cv2.COLOR_RGB2LAB)[:, :, 1:] / 110.0
        else:
            ref_rgb = resize(ref_rgb, size=(224, 224))
            ref_rgb = (np.array(ref_rgb) / 255.0).astype(np.float32)

            img_rgb = resize(img_rgb, size=(224, 224))
            img_rgb = (np.array(img_rgb) / 255.0).astype(np.float32)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            img_spatial_aug = (np.array(img_spatial_aug) / 255.0).astype(np.float32)
            img_pixel_aug = (np.array(img_pixel_aug) / 255.0).astype(np.float32)
            img_ref = (np.array(img_rgb_aug) / 255.0).astype(np.float32)

            img_l = cv2.cvtColor(img_pixel_aug, cv2.COLOR_RGB2LAB)[:, :, 0] / 100.0
            img_ab = cv2.cvtColor(img_spatial_aug, cv2.COLOR_RGB2LAB)[:, :, 1:] / 110.0
        else:
            img_rgb = resize(img_rgb, size=(224, 224))
            img_ref = (np.array(img_rgb) / 255.0).astype(np.float32)
