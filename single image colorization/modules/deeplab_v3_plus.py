...
class _XceptionBlock(nn.Module):
    def __init__(
        ...
        conv_separable = partial(
            Conv2dSeparable, kernel_size=3, dilation=dilation, relu_first=relu_first
        )
...
class _Xception(nn.Module):
    ...
    def __init__(self):
        ...
        self.entry_block1 = _XceptionBlock([64, 128, 128, 128], stride=2)

        self.entry_block2 = _XceptionBlock([128, 256, 256, 256], stride=2, ll=True)

        self.entry_block3 = _XceptionBlock(
            [256, 728, 728, 728], stride=self.ENTRY_STRIDE
        )
        ...
        self.exit_block1 = _XceptionBlock(
            [728, 728, 1024, 1024], dilation=self.EXIT_DILATIONS[0]
        )

        self.exit_block2 = _XceptionBlock(
            [1024, 1536, 1536, 2048],
...
class _Decoder(nn.Module):
    def __init__(
        self, ll_in_channels, ll_reduced_channels, enc_in_channels, enc_out_channels
    ):
        ...
        self.conv2 = Conv2dSeparable(
            in_channels=enc_in_channels, out_channels=enc_out_channels, kernel_size=3
        )
...
class _TFConverter:
    def __init__(self, tf, checkpoint):
        self.tf_reader = tf.compat.v1.train.NewCheckpointReader(checkpoint)
        self.prefix = None
    ...
    def batchnorm(self, pt_layer, tf_layer):
        pt_layer.bias.data = self._get(pt_layer.bias.data, tf_layer, "beta")
        pt_layer.weight.data = self._get(pt_layer.weight.data, tf_layer, "gamma")
...
class DeepLabV3Plus(nn.Module):
    ...
    def __init__(self, out_channels):
        ...
        self.aspp = _AtrousSpatialPyramidPooling(
            in_channels=self.ASPP_IN_CHANNELS, out_channels=self.ASPP_OUT_CHANNELS
        )
    ...
    def init_from_tensorflow(self, checkpoint, xception_only=False, init_logits=False):
